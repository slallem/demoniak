...
| **Ollama** 🏠 | `_01` starter · `_02` chat, both via the OpenAI-compatible API and the native HTTP API · `_03` 🔧 tool calling (native `/api/chat`) · `_04` structured outputs (native `format` + JSON Schema) · `_05` 🔎 local embeddings + semantic RAG over Sherlock Holmes · `_06` 👁️ vision (native `images` field) · `_07` 📊 local perf introspection (`load_duration`, tokens/s, `/api/ps`) — see [`doc/ollama.md`](doc/ollama.md) for the two API surfaces and local model picks |
